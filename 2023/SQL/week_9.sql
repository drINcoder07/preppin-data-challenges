...
CREATE TEMPORARY VIEW account_information AS
SELECT account_number, balance_date, balance
FROM year2023.wk_7_account_information;
...
CREATE TEMPORARY VIEW incoming_transactions AS
SELECT p.account_to, d.transaction_date, d.value
...
, merged_values AS
(SELECT a.account_number, a.balance_date, a.balance AS combined_values, ABS(a.balance) as abs_values, COALESCE(i.value, 0) + COALESCE(o.value, 0) AS transaction_value, ai.balance
FROM combined_tables a
LEFT JOIN account_information ai ON a.account_number = ai.account_number AND a.balance_date = ai.balance_date AND   a.balance = ai.balance
LEFT JOIN incoming_transactions i ON a.account_number = i.account_to AND a.balance_date = i.transaction_date AND    a.balance = i.value
