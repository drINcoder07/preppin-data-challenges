...
CREATE TEMPORARY VIEW merged_locations AS
SELECT *
FROM year2023.wk_11_customer_locations , 
      LATERAL (SELECT * FROM year2023.wk_11_branch_locations);
...
(SELECT
*,
RANK() OVER(PARTITION BY customer ORDER BY distance) AS ranking
FROM with_distance
ORDER BY customer, ranking
)
SELECT
branch,
branch_long,
